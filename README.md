...
“识别疑似导航页，并在导出前确认跳过清单”默认开启。程序只分析确实有子文档的新版文档：连续出现至少 3 个非空正文块、正文累计达到约 100 字，或包含表格、图片、附件等明确内容时，会保留导出；标题、空行、分割线以及新旧版子页面列表不算实际正文。未达到这些条件的页面会进入审核清单，默认勾选为跳过，用户可以取消勾选让它照常导出；遇到未知块类型或分析失败时也会保留导出，避免误删正文。
