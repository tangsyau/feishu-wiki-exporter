...
- 读取目录和内嵌附件时显示扫描进度；
- 导出完成后可自动生成供 Tauri 阅读器使用的离线知识库；
- 离线知识库保留目录层级、原文件，并为 DOCX 建立中文全文索引；
- JSON 状态文件保存成功、失败和修改时间，支持中断后继续；
...
不要删除 `.feishu-export/state.json`，否则程序无法判断哪些文件自上次导出后没有修改。默认采用“增量判断 + 覆盖已有文件”：未修改的项目会跳过，已修改的项目会更新本地文件。知识库导出还会在同一内部目录生成 `order.json`，保存飞书节点的父子关系、同级顺序及对应导出路径，不会修改实际文件名。
...
审核采用三级分类：“高度疑似导航页”默认勾选跳过；未知块、分析失败或疑似由页面链接组成的表格归入“无法确定”，显示在同一清单中但默认保留；明确有正文的页面不进入跳过清单。用户仍可逐项改变选择。每次确认导出后，详细的块类型数量、正文指标、分类理由和错误会写入 `.feishu-export/navigation-analysis.json`，用于排查误判；该文件不包含 App Secret。
...
在“导出设置”中保持“导出完成后生成离线知识库”勾选，正常导出结束后会在原知识库目录旁边生成 `<知识库名>-offline`。
...
生成器会将 DOCX 中的常见标题、段落、列表、表格和内嵌图片转换为网页阅读内容，并建立二元组中文全文索引。知识库目录树直接使用飞书节点关系：一个页面可以同时拥有正文和子页面，Reader 中标题用于打开正文，箭头用于展开子页面；纯导航页则只有子页面。导航页分析只决定是否导出正文文件，不再决定目录层级。实际文件名不会添加序号。缺少新版层级元数据时，仍会按旧版文件目录和名称生成兼容目录树。只在飞书中调整节点顺序而不修改正文时，下一次导出仍可跳过未变化文件，但重新生成的离线知识库会采用新顺序。搜索结果采用严格分组：标题命中的结果始终排在仅正文命中的结果之前；组内再按匹配程度排序。从搜索结果打开文档后，可以返回原结果列表，并恢复搜索关键词和浏览位置。PDF、XLSX 和其他附件保留原文件，在阅读器中可通过标题找到并调用系统默认程序打开。扫描版 PDF 的 OCR 当前不在支持范围。
...
- 不支持 Markdown；
- 离线阅读器只对 DOCX 建立正文全文索引；PDF、表格和其他附件可以按标题查找并打开原文件，但暂不解析正文，也不做 OCR；
